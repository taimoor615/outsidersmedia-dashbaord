...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            ...
            <a href="{{ route('posts.index', ['status' => 'draft']) }}" class="text-xs font-medium mt-1 inline-block" style="color:#CD571B;">Continue editing →</a>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs font-semibold px-2 py-1 rounded-full" style="color:#CD571B;background:#FEE8DC;">Action Needed</span>
                @endif
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $stats['changes_requested'] }}</h3>
            <p class="text-sm text-gray-600">Needs Changes</p>
            @if($stats['changes_requested'] > 0)
            <a href="{{ route('posts.index', ['status' => 'changes_requested']) }}" class="text-xs font-medium mt-1 inline-block" style="color:#CD571B;">Fix now →</a>
            @endif
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ route('posts.create') }}" class="flex items-center gap-4 p-4 border rounded-xl hover:shadow-md transition-all" style="background:#FEF3EC;border-color:#F5C4A0;">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background:linear-gradient(135deg,#CD571B,#EC921A);">
            ...
            <a href="{{ route('clients.index') }}" class="flex items-center gap-4 p-4 border rounded-xl hover:shadow-md transition-all" style="background:#FEF3EC;border-color:#F5C4A0;">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background:linear-gradient(135deg,#CD571B,#EC921A);">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('posts.index') }}" class="flex items-center gap-4 p-4 border rounded-xl hover:shadow-md transition-all" style="background:#FEF3EC;border-color:#F5C4A0;">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background:linear-gradient(135deg,#CD571B,#EC921A);">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="divide-y divide-gray-200">
                ...
                <div class="p-6 hover:bg-gray-50 transition-colors">
                    <div class="flex items-start gap-4">
                        ...
                        <img src="{{ $post->media->first()->url }}" alt="Post" class="w-16 h-16 rounded-xl object-cover flex-shrink-0">
                        @else
                        <video src="{{ $post->media->first()->url }}" class="w-16 h-16 rounded-xl object-cover flex-shrink-0" muted playsinline></video>
                        @endif
                        @else
                        ...
                        <div class="flex-1">
                            ...
                            <div class="p-2 rounded-lg mb-2" style="background:#FEF3EC;border:1px solid #F5C4A0;">
                                <p class="text-xs" style="color:#7C2D12;">
                                    <strong>Feedback:</strong> {{ Str::limit($post->feedback->where('is_client_feedback', true)->last()->feedback, 100) }}
                                </p>
                            </div>
                            @endif
                            <a href="{{ route('posts.edit', $post) }}" class="text-sm font-medium" style="color:#CD571B;">Edit Post →</a>
                        </div>
                    </div>
                </div>
                @endforeach
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between mb-1">
                                ...
                                <span class="px-2 py-1 text-xs font-semibold rounded-full" style="background:#FEE8DC;color:#CD571B;">Changes</span>
                                @endif
                            </div>
            ...
            <div class="divide-y divide-gray-200">
                ...
                <div class="p-6 hover:bg-gray-50 transition-colors">
                    <div class="flex items-start gap-4">
                        @if($post->media->count() > 0)
                        @if($post->media->first()->isImage())
                        <img src="{{ $post->media->first()->url }}" alt="Post" class="w-20 h-20 rounded-xl object-cover flex-shrink-0">
                        @else
                        <video src="{{ $post->media->first()->url }}" class="w-20 h-20 rounded-xl object-cover flex-shrink-0" muted playsinline></video>
                        @endif
                        @else
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-4 text-sm">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <span class="px-2 py-1 text-xs font-medium rounded-lg capitalize" style="background:#FEE8DC;color:#CD571B;">{{ $platform }}</span>
                                    @endforeach
                                </div>
                            </div>
                        </div>
    ...
    }" x-init="$nextTick(() => renderTeamCalendar(calMonth, calYear))"
    x-effect="$nextTick(() => renderTeamCalendar(calMonth, calYear))">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-gray-900">Scheduled Posts Calendar</h2>
                <p class="text-sm text-gray-500">All scheduled posts across all clients</p>
            </div>
        </div>
        <div class="p-6">
            <!-- Month Navigation -->
            ...
            <!-- Calendar Grid -->
            <div class="grid grid-cols-7 gap-1" id="teamCalGrid"></div>
            <!-- Day Posts Detail -->
            <div id="teamCalDayPosts" class="mt-4 space-y-2"></div>
        </div>
    </div>

    <!-- Your Responsibilities Note -->
    <div class="rounded-xl p-6" style="background:#FEF3EC;border:1px solid #F5C4A0;">
        <div class="flex gap-4">
            <svg class="w-6 h-6 flex-shrink-0" style="color:#CD571B;" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
            </svg>
            <div>
                <h3 class="font-bold mb-2" style="color:#7C2D12;">Your Responsibilities</h3>
                <ul class="space-y-1 text-sm" style="color:#92400E;">
...
@push('scripts')
<script>
const teamPostDates = @json($calendarPosts);

window.renderTeamCalendar = function(month, year) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const dayPosts = teamPostDates.filter(p => p.date === dateStr);
        const isToday = today.getDate()===d && today.getMonth()===month && today.getFullYear()===year;

        const btn = document.createElement('button');
        btn.textContent = d;
        btn.className = 'w-full text-center py-2 text-sm rounded-lg transition';
        ...
        btn.addEventListener('click', () => {
            const container = document.getElementById('teamCalDayPosts');
            if (!dayPosts.length) {
                container.innerHTML = '<p class="text-sm text-gray-400 text-center py-2">No posts scheduled on this day.</p>';
                return;
            }
            container.innerHTML = `<p class="text-xs font-semibold text-gray-500 mb-2">Posts on ${dateStr}</p>` +
                dayPosts.map(p => `
                    <div class="p-3 border rounded-xl text-sm text-gray-800 flex items-start justify-between gap-2" style="background:#FEF3EC;border-color:#F5C4A0;">
                        <div>
                            <span class="font-semibold" style="color:#CD571B;">${p.client}</span>
                            <span class="mx-1 text-gray-400">·</span>
                            <span>${p.title.substring(0,70)}${p.title.length>70?'...':''}</span>
                        </div>
                        <span class="text-xs capitalize whitespace-nowrap" style="color:#CD571B;">${p.status.replace(/_/g,' ')}</span>
                    </div>
                `).join('');
        });
